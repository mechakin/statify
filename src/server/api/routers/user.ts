...
async function getUserData(timePeriod: TimePeriod, accessToken: string) {
  const responseTopSongs: AxiosResponse<SongData> = await axios.get(
    `https://api.spotify.com/v1/me/top/tracks?time_range=${timePeriod}_term&limit=50`,
    {
      headers: {
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const createPlaylistResponse: AxiosResponse<PlaylistData> =
        await axios.post(
          ...
          { name: "recommended for you", description: "automatically generated by statify", public: false },
          {
            headers: {
              Authorization: `Bearer ${account?.access_token}`,
            },
